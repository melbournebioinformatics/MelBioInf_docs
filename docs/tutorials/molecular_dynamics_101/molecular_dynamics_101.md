...
VMD can be freely obtained from:

	http://www.ks.uiuc.edu/Research/vmd/

Additional tutorials and information about NAMD are available at:

	http://www.ks.uiuc.edu/Research/namd/


1. Logging in
Using a terminal on your local computer and the <username> and password given to you at the workshop, login to the HPC cluster.
...
Special note about terminals:
Be careful to use the right terminal when you are typing in commands!
...
`[train01@snowy ~]$`               <- tells you your terminal is on snowy


2. Copy across files, starting the job
...
3) Understanding the input files.
...
For example, in Linux  you can type in your local computer terminal: (if you see “avoca” in the command line prompt you are typing in the wrong terminal!)
...
scp -r <user>@snowy.vlsci.org.au:vlsci/examples/namd/
Namd_simple_example_01_finished .  
(all on one line!)  
...
(or start from the Program menu if you are using a Windows machine, (under the folder University of Illinios)
If this doesn’t work, there could be a problem with defining the path to vmd on your computer.
...
The terminal should turn into the console:
...
1) Reading structure data files into VMD
